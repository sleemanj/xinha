...
function InsertSnippet(editor) {
    ...
    Xinha._getback(cfg.InsertSnippet.snippets,function (txt,req) {
...
{
  'snippets' : _editor_url+"plugins/InsertSnippet/snippets.xml",
...
InsertSnippet.prototype._prepareDialog = function()
{
  ...
  {
    Xinha._getback(_editor_url + 'plugins/InsertSnippet/dialog.html', function(getback) { self.html = getback; self._prepareDialog(); });
    return;
  }
  if (!this.snippets)
  {
  ...
  var InsertSnippet = this;

  this.dialog = new Xinha.Dialog(editor, this.html, 'InsertSnippet',{width:800,height:400},{modal:true});
...
InsertSnippet.prototype.preview = function(event)
{
	if (!event)
	{
...
InsertSnippet.prototype.doInsert = function(event)
{
    var target = event.target || event.srcElement;
