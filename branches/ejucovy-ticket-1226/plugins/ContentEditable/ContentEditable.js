function ContentEditable(editor) {
  xinha_editor = this.editor = editor;
...
// xinha_editor.getSelection() on the context object. (Thus the active range may be null.)"
//
...
function getActiveRange() {
	...
	if (globalRange) {
		ret = globalRange;
	} else if (xinha_editor.getSelection().rangeCount) {
		ret = xinha_editor.getSelection().getRangeAt(0);
	} else {
		return null;
	}
...
function movePreservingRanges(node, newParent, newIndex) {
	...
	for (var i = 0; i < xinha_editor.getSelection().rangeCount; i++) {
		ranges.push(xinha_editor.getSelection().getRangeAt(i));
	}
	var boundaryPoints = [];
	...
	xinha_editor.getSelection().removeAllRanges();
	for (var i = 1 + extraRanges.length; i < ranges.length; i++) {
		...
		xinha_editor.getSelection().addRange(newRange);
	}
}
...
function setSelectionValue(command, newValue) {
	...
	&& getActiveRange().startOffset != getNodeLength(getActiveRange().startContainer)) {
		...
		xinha_editor.getSelection().removeAllRanges();
		xinha_editor.getSelection().addRange(newActiveRange);
	...
	&& getActiveRange().endOffset != getNodeLength(getActiveRange().endContainer)) {
		...
		xinha_editor.getSelection().removeAllRanges();
		xinha_editor.getSelection().addRange(activeRange);
	}
...
function deleteSelection(flags) {
	...
	if (getPosition(endNode, endOffset, startNode, startOffset) !== "after") {
		...
		if (direction == "forward") {
			if (xinha_editor.getSelection().rangeCount) {
				xinha_editor.getSelection().collapseToStart();
			}
			getActiveRange().collapse(true);

		// "Otherwise, call collapseToEnd() on the context object's Selection."
		} else {
			xinha_editor.getSelection().collapseToEnd();
	...
	if (isAncestor(startBlock, endBlock)) {
		...
		// node."
		xinha_editor.getSelection().collapse(startBlock, getNodeIndex(referenceNode));
	...
	// "Otherwise, if start block is a descendant of end block:"
		...
		xinha_editor.getSelection().collapse(startBlock, getNodeLength(startBlock));
...
function autolink(node, endOffset) {
	...
	xinha_editor.getSelection().removeAllRanges();
	xinha_editor.getSelection().addRange(originalRange);
...
commands["delete"] = {
	...
	action: function() {
		...
		&& isHtmlElement(node.childNodes[offset - 1], ["br", "hr", "img"]))) {
			...
			xinha_editor.getSelection().collapse(node, offset);
			getActiveRange().setEnd(node, offset);

			// "Call extend(node, offset − 1) on the context object's
			// Selection."
			xinha_editor.getSelection().extend(node, offset - 1);
		...
		&& isHtmlElement(startNode.childNodes[startOffset - 1], "table")) {
			...
			// object's Selection."
			xinha_editor.getSelection().collapse(startNode, startOffset - 1);
			getActiveRange().setStart(startNode, startOffset - 1);

			// "Call extend(start node, start offset) on the context object's
			// Selection."
			xinha_editor.getSelection().extend(startNode, startOffset);
		...
		&& isHtmlElement(startNode.childNodes[startOffset].previousSibling, ["li", "dt", "dd"])) {
			...
			xinha_editor.getSelection().extend(node, 0);
		...
		xinha_editor.getSelection().extend(node, offset);
		getActiveRange().setEnd(node, offset);
...
commands.forwarddelete = {
	...
	action: function() {
		...
		&& offset != getNodeLength(node)) {
			...
			// Selection."
			xinha_editor.getSelection().extend(node, endOffset);
		...
		if (isHtmlElement(endNode.childNodes[endOffset], "table")) {
			...
			// Selection."
			xinha_editor.getSelection().collapse(endNode, endOffset);
			getActiveRange().setStart(endNode, endOffset);

			// "Call extend(end node, end offset + 1) on the context object's
			// Selection."
			xinha_editor.getSelection().extend(endNode, endOffset + 1);
...
commands.inserthorizontalrule = {
	...
	action: function() {
		...
		&& getActiveRange().startOffset == 0) {
			...
			xinha_editor.getSelection().collapse(newNode, newOffset);
		...
		// index."
		xinha_editor.getSelection().collapse(hr.parentNode, 1 + getNodeIndex(hr));
...
commands.insertimage = {
	...
	action: function(value) {
		...
		xinha_editor.getSelection().removeAllRanges();
		xinha_editor.getSelection().addRange(range);
...
commands.insertlinebreak = {
	...
	action: function(value) {
		...
		// argument."
		xinha_editor.getSelection().collapse(br.parentNode, 1 + getNodeIndex(br));
	...
	action: function() {
		...
		|| !isSingleLineContainer(container)) {
			...
			if (!nodeList.length) {
				...
				// Selection."
				xinha_editor.getSelection().collapse(container, 0);
		...
		|| container.tagName == "PRE") {
			...
			// Selection."
			xinha_editor.getSelection().collapse(node, offset + 1);
			getActiveRange().setStart(node, offset + 1);
			getActiveRange().setEnd(node, offset + 1);
		...
		xinha_editor.getSelection().collapse(newContainer, 0);
...
commands.inserttext = {
	action: function(value) {
		...
		if (node.nodeType == Node.TEXT_NODE) {
			...
			try { xinha_editor.getSelection().extend(node, offset + 1); } catch(e) {}
			getActiveRange().setEnd(node, offset + 1);
		...
		try { xinha_editor.getSelection().collapseToEnd(); } catch(e) {}
		getActiveRange().collapse(false);
...
commands.selectall = {
	...
	action: function() {
		...
		// "If target is null, call xinha_editor.getSelection() on the context object, and
		// call removeAllRanges() on the result."
		if (!target) {
			xinha_editor.getSelection().removeAllRanges();

		// "Otherwise, call xinha_editor.getSelection() on the context object, and call
		// selectAllChildren(target) on the result."
		} else {
			xinha_editor.getSelection().selectAllChildren(target);
		}
...
//@}
  this.__onExecCommand = function(cmd, ui, value) {
    return myExecCommand(cmd, ui, value);
  };
  this.__nextNode = function(node) { return nextNode(node); };
// vim: foldmarker=@{,@} foldmethod=marker
};

ContentEditable.prototype.onGenerate = function() {
  console.log(this);
};
ContentEditable.prototype.onKeyPress = function(evt) {

  if( evt.keyCode == 13 ) {
    Xinha._stopEvent(evt);
    var range = this.editor.createRange(this.editor.getSelection()),
        startContainer = range.startContainer,
        startOffset = range.startOffset,
        endContainer = range.endContainer,
        endOffset = range.endOffset;

    var activeEl = maybeBlockEl = this.editor.getParentElement();

    // Find the innermost block element, between the active element (inclusive) 
    // and the editing host (exclusive), containing the active element. 
    // If there's no such containing block, wrap the active element with a <P> tag,
    // and if either the previous or next sibling is missing, stuff in a <SPAN> 
    // or two for the hell of it.
    var needsWrapping = true;

    while( needsWrapping ) {

      var isBlockElement = Xinha.isBlockElement(maybeBlockEl);
      if( isBlockElement ) {
          break;
      }
      if( !maybeBlockEl.parentNode
        || maybeBlockEl.parentNode == this.editor._doc.body
        || maybeBlockEl.parentNode == this.editor._doc  ) {
          activeEl = maybeBlockEl;
          maybeBlockEl = null;
          break;
      }
      maybeBlockEl = maybeBlockEl.parentNode;
    }
    if( !maybeBlockEl ) {
      maybeBlockEl = this.editor._doc.createElement("p");
      activeEl.parentNode.replaceChild(maybeBlockEl, activeEl);
      maybeBlockEl.appendChild(activeEl);
      var new_range = this.editor.createRange();
      new_range.setStart(startContainer, startOffset);
      new_range.setEnd(endContainer, endOffset);
      this.editor.restoreSelection(new_range);
    }
    if( maybeBlockEl.nextSibling == null ) {
      maybeBlockEl.parentNode.insertBefore(this.editor._doc.createElement('span'), null);
    }
    return this.__onExecCommand("insertParagraph");
  }
};
